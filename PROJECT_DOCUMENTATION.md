...
This project automates employee leave management. Employees can register, log in, apply for leave, and view leave history. Admins can view all leave requests and approve or reject them.

### 2. What type of application is this?

It is a Spring Boot web application using Spring MVC, Thymeleaf, Spring Data JPA, and MySQL.

### 3. What is MVC?

MVC means Model-View-Controller. In this project, model means entities and DTOs, view means Thymeleaf HTML pages, and controller means Java classes that handle requests.

### 4. Which class starts the application?

`EmployeeLeaveManagementSystemApplication.java` starts the application. It contains the `main` method and uses `@SpringBootApplication`.

### 5. What does `@SpringBootApplication` do?

It marks the main class as a Spring Boot application. It enables auto-configuration, component scanning, and configuration support.

### 6. What are the main entities?

The main entities are `Employee` and `LeaveRequest`.

### 7. Which tables are used?

The database uses `employee` and `leave_request` tables.

### 8. What is the relationship between Employee and LeaveRequest?

One employee can have many leave requests. Each leave request belongs to one employee.

### 9. How is this relationship represented in Java?

`Employee` has `@OneToMany`, and `LeaveRequest` has `@ManyToOne`.

### 10. What is a DTO?

A DTO is a Data Transfer Object. It carries form data between the HTML page and controller.

### 11. Why are DTOs used instead of entities in forms?

DTOs prevent direct binding to database entities, improve security, and keep validation specific to form input.

### 12. Name the DTO classes in this project.

The DTO classes are `EmployeeRegistrationDto`, `LoginDto`, and `LeaveRequestDto`.

### 13. What does `EmployeeRegistrationDto` do?

It stores registration form data such as full name, email, password, department, and designation.

### 14. What does `LoginDto` do?

It stores login form data: email and password.

### 15. What does `LeaveRequestDto` do?

It stores leave application form data: leave type, start date, end date, and reason.

### 16. What is `@NotBlank`?

`@NotBlank` validates that a text field is not empty and not only spaces.

### 17. What is `@Email`?

`@Email` validates that a field contains a proper email format.

### 18. What is `@FutureOrPresent`?

It validates that a date is today or in the future.

### 19. How is duplicate employee registration prevented?

`EmployeeServiceImpl` calls `employeeRepository.existsByEmail()` before saving a new employee.

### 20. How is duplicate leave prevented?

`LeaveRequestRepository.existsOverlappingLeave()` checks whether a pending or approved leave already overlaps the requested dates.

### 21. Why are rejected leaves ignored in duplicate checking?

Because if a leave was rejected, the employee should be allowed to apply again for those dates.

### 22. What is a repository?

A repository is a database access layer. It provides methods to save, find, count, and query data.

### 23. What does `JpaRepository<Employee, Long>` mean?

It means the repository works with `Employee` objects, and the primary key type is `Long`.

### 24. What is Spring Data JPA?

Spring Data JPA is a library that simplifies database access by creating repository implementations automatically.

### 25. What is `@Entity`?

`@Entity` marks a Java class as a database table mapping.
...
### 28. What is `@GeneratedValue`?

It tells JPA that the database should generate the primary key value automatically.

### 29. What is `@Enumerated(EnumType.STRING)`?

It stores enum values as text in the database, such as `APPROVED`, instead of numbers.

### 30. What is the service layer used for?

The service layer contains business logic such as registration rules, authentication, leave validation, approval, and rejection.

### 31. What does `@Service` do?

It marks a class as a Spring service component so Spring can create and inject it.

### 32. What does `@Controller` do?

It marks a class as a Spring MVC controller that can handle web requests.

### 33. What does `@GetMapping` do?

It maps HTTP GET requests to a controller method.

### 34. What does `@PostMapping` do?

It maps HTTP POST requests to a controller method, usually for form submission.

### 35. What is Thymeleaf used for?

Thymeleaf is used to create dynamic HTML pages that display backend data.

### 36. What does `th:object` do?

It binds a Thymeleaf form to a Java object, usually a DTO.

### 37. What does `th:field` do?

It binds an input field to a property of the form object.

### 38. What does `th:text` do?

It displays a value from the backend model in the HTML page.

### 39. How does login work in this project?

The user submits email and password. The service finds the employee by email, checks the password, creates a session, and redirects based on role.

### 40. How is role-based navigation handled?

If the role is `ADMIN`, the user goes to `/admin/dashboard`. If the role is `EMPLOYEE`, the user goes to `/employee/dashboard`.

### 41. Does this project use Spring Security?

No. It uses simple manual session handling. Spring Security is a future enhancement.

### 42. What is a session?

A session stores user information on the server between requests. This project stores employee ID, admin ID, name, and role.

### 43. What happens when the user logs out?

The session is invalidated, which removes login information, and the user is redirected to the login page.

### 44. What is `application.properties` used for?

It stores application configuration such as database URL, username, password, JPA settings, Thymeleaf cache setting, and server port.

### 45. What is Maven used for?

Maven manages dependencies, compiles the project, runs tests, and starts the Spring Boot application.

### 46. What is Lombok used for?

Lombok reduces repeated Java code by generating getters, setters, constructors, and builders.

### 47. What does `@RequiredArgsConstructor` do?

It generates a constructor for all final fields. Spring uses that constructor for dependency injection.

### 48. What is dependency injection?

Dependency injection means Spring automatically provides required objects, such as repositories inside services and services inside controllers.

### 49. What is `GlobalExceptionHandler`?

It is a central class that handles exceptions from controllers and returns the error page.

### 50. How can this project be improved?

It can be improved by adding Spring Security, password hashing, leave balance, email notifications, pagination, search, file upload, and REST APIs.
